...
void embedMarkerAtPosition(HPDF_Doc pdf, HPDF_Page page, const cv::Mat& marker, float x, float y) {
    // Convert marker to RGB format
    cv::Mat markerRGB;
    cv::cvtColor(marker, markerRGB, cv::COLOR_GRAY2RGB);

    // Create raw image data buffer for libharu
    std::vector<unsigned char> imageData(markerRGB.total() * markerRGB.elemSize());
    std::memcpy(imageData.data(), markerRGB.data, imageData.size());

    HPDF_Image image = HPDF_LoadRawImageFromMem(
        pdf,
        imageData.data(),
        markerRGB.cols,
        markerRGB.rows,
...
int main() {
    try {
        ...
        std::vector<std::pair<float, float>> positions = {
            ...
            {pageWidth - margin - markerSize, pageHeight/2 - margin - markerSize/2}, // Right-middle
            {pageWidth/2 - margin - markerSize/2, margin}, // Bottom-middle 
            {margin, pageHeight - margin - markerSize}, // Top-left corner
            {pageWidth - margin - markerSize, pageHeight - margin - markerSize}, // Top-right corner
            {pageWidth/2 - margin - markerSize/2, pageHeight - margin - markerSize} // Top-middle
        };

        for (size_t i = 0; i < markers.size(); ++i) {
            embedMarkerAtPosition(pdf, page, markers[i], positions[i].first, positions[i].second);
        }
